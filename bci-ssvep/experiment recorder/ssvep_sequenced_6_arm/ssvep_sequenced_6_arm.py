...
class Settings:
    ...
    def __init__(self):
        ...
        self.was_correct_key = 'arrow_right'
        self.was_incorrect_key = 'arrow_left'
        ...
        self.arm_direction_map = {'8hz': 'ROTATE_LEFT',
                                  '9hz': 'ROTATE_RIGHT',
                                  '13hz': 'EXTEND_HORIZONTAL',
                                  '14hz': 'RETRACT_HORIZONTAL',
                                  '15hz': 'GRAB',
                                  '16hz': 'RELEASE'}
...
class Main(LatentModule):
    ...
    def run(self):
        ...
        for trial_idx, frequency in enumerate(self.all_trials):
            ...
            process_trial_request = {'markers': [f'trial-start-{frequency}-hz-{trial_idx}', f'trial-end-{frequency}-hz-{trial_idx}']}
            result = requests.post('http://localhost:5000/api/process/process-trial/guess-only', json=process_trial_request)

            if result.status_code != 200:
                self.write("Processing of trial failed with error, exiting...", duration=2, block=True, align='center', pos=(0, 0), scale=0.12)
                return
            
            data = result.json()
            guessed_frequency = data['guessedFrequencyMix']

            guess_text = self.write(f'''Guessed - {guessed_frequency}
            
                                        Press RIGHT ARROW if guess was correct(arm will move), otherwise press LEFT ARROW''',
                                    fg=self.color_map['black'], duration=0, block=False, align='center', scale=0.09, wordwrap=30, pos=[0, 0.4])

            was_correct_or_not = self.waitfor_multiple([self.was_correct_key, self.was_incorrect_key])

            if was_correct_or_not[0] == self.was_correct_key:
                logger.info("Send was correct")
                requests.post(f'http://localhost:5000/api/arm-control/move/direction/{self.arm_direction_map[guessed_frequency]}')
                requests.post(f'http://localhost:5000/api/process/process-trial/guess-only/result/CORRECT')
            elif was_correct_or_not[0] == self.was_incorrect_key:
                logger.info("Send was IN-correct")
                requests.post(f'http://localhost:5000/api/process/process-trial/guess-only/result/INCORRECT')
            else:
